...
const botao = document.querySelector('.btn-icon')
const idConselho = document.querySelector('.advice-id')
const idDescricao = document.querySelector('.mensage')
console.log(botao)
...
async function exibirConselho(){
    ...
    const id =  `Advice #${conselho.slip.id}`
    const desc =  `${conselho.slip.advice}`
    idConselho.innerHTML = id
    idDescricao.innerHTML = desc
}
